...
trait BazelPluginExtension
  extends BazelOpenProjectFixture
    with RobotPluginExtension { this: IdeProbeFixture =>

  if (!System.getProperty("java.version").startsWith("11.")) {
    throw new AssertionError("Bazel tests must run on java 11. Also make sure JAVA_HOME points to java 11")
  }
